...
func TestLoadPilotSeedsFreshStore(t *testing.T) {
	...
	r := h.ok("load", "pilot", "--task", "start", "--meta", "repo-id=x", "--meta", "harness=test", "--budget", "4000")
...
func TestLoadPilotKeepsExistingAgents(t *testing.T) {
	...
	if r := h.ok("load", "reflector", "--budget", "4000"); !strings.Contains(r.out, "Mine, not the starter.") {
	...
	r = h2.ok("load", "pilot", "--budget", "4000")
